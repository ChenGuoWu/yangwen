  ...
  <div class="organization">
      ...
      <div class="header">
        <div class="query_info">
          <div class="query_item">
            <span>管理办法名称</span>
            <el-input
              v-model="queryForm.managementMethodName"
        ...
        <div>
          <template v-if="userRole" class="rol">
      ...
      <div class="table_content" v-loading="loading">
        <div class="table_box">
          ...
          >
            ...
            <el-table-column prop="releaseTime" label="发文时间" width="150">
            </el-table-column>
            <el-table-column
              :show-overflow-tooltip="true"
            ...
            >
              <template slot-scope="scope">
                <span
                  :style="{
                    color:
                      scope.row.managementMethodStatus == 1
                        ? '#00479D'
                        : '#333333',
                  }"
                  >{{
                    scope.row.managementMethodStatus == "1" ? "有效" : "无效"
                  }}</span
                >
              </template>
            </el-table-column>
    ...
    >
      <el-form
        ...
        label-width="60px"
      >
        <el-form-item prop="managementMethodName" label="管理办法名称">
          <el-input
            v-model="addForm.managementMethodName"
            placeholder="请输入管理办法名称"
            class="inputs"
          ></el-input>
        </el-form-item>
        ...
        <template v-if="userRole">
          ...
          <el-form-item prop="responsibleDeptName" label="所属部门">
            <el-select
              clearable
              class="search_query width_2rem"
              placeholder="请选择所属部门"
        ...
        <el-form-item label="状态" class="formpower">
          <el-radio-group v-model="addForm.managementMethodStatus">
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      delete data.pubTime;
      this.$http
        .post("/rule/managementMethod/getManagementMethodByCondition", data)
        .then((res) => {
          this.loading = false;
          if (res.code == "1") {
  ...
  //下拉框清空按钮
    ...
    publishData() {
      if (!this.editId) {
        ...
        this.uploadData.append(
          "managementMethodStatus",
          this.addForm.managementMethodStatus
        );
        this.$http
          .post(
            "/rule/managementMethod/addRuleManagementMethod",
            this.uploadData,
            {
              headers: {
        ...
        this.$http
          .post(
            "/rule/managementMethod/updateRuleManagementMethod",
            this.uploadData,
            {
              headers: {
                "Content-Type": "application/x-www-form-urlencoded",
    ...
    resizeForm() {
      ...
      //重置
      this.addForm.managementMethodStatus = "0";
    },
    //编辑数据
    editData(row) {
      console.log(row)
    ...
    deleteData(row) {
      this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          this.$http
            .get("/rule/managementMethod/delRuleManagementMethodById", data)
            .then((res) => {
              if (res.code == "1") {
                this.$message({
                  type: "success",
