  ...
  <div class="organization">
    <div class="one_level">
      ...
      <div class="table_content" v-loading="loading">
        ...
        >
          <el-table-column type="index" label="序号" width="200" align="left">
          </el-table-column>
          <el-table-column
    ...
    >
      <el-form
        ...
        label-width="60px"
      >
        <el-form-item prop="dicName" label="字典名称" class="contents">
        </el-form-item>
        <el-input v-model="addForm.dicName" class="inputs"></el-input>
        <el-form-item prop="dicTypeDes" label="字典描述" class="contents">
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.$http
        .get("/rule/dataDicType/searchDicTypeByName", data)
        .then((res) => {
          this.loading = false;
          if (res.code == "1") {
      ...
      this.$refs["addForm"].validate((valid) => {
        if (valid) {
          ...
          if (!this.editId) {
            ...
            this.$http
              .post("/rule/dataDicType/addRuleDicType", data)
              .then((res) => {
                if (res.code == "1") {
            ...
            data.id = this.editId;
            this.$http
              .post("/rule/dataDicType/updateRuleDicType", data)
              .then((res) => {
                if (res.code == "1") {
                  this.popupLoading = false;
                  this.showPopup = false;
    ...
    deleteData(row) {
      this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          this.$http
            .get("/rule/dataDicType/deleteRuleDicType", data)
            .then((res) => {
              if (res.code == "1") {
                this.$message({
                  type: "success",
...
this.$store.dispatch('changeIsAddRoute', true);
      //根据value.dicName拿到对应的value.dicType,
      this.$router.push({
        name: "Managementofthekeyvalue",
        query: {
          dicType: value.dicTypeCode, //所属数据字典类型
          dicName: value.dicName, //值（数据字典的名称）
        },
      });
    },
  },
