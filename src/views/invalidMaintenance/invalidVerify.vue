<template>
	<div class="invalidApply">
				...
				<div class="table_box">
					<el-table  height="100%" :header-cell-style="{background:'#E1E8F4',color:'#333333',fontWeight:'bold'}" :data="tableData" :cell-style="{padding: '0'}" :row-style="{height: '0.4rem'}" style="width: 100%; margin-bottom: 0.2rem;">
						...
						<el-table-column align="center" label="操作" width="170" fixed="right">
							<template slot-scope="scope">
								<el-button :disabled="scope.row.auditStatus == '2' || !scope.row.auditStatus || scope.row.auditStatus == '3'" :style="{color: (scope.row.auditStatus == '2' || !scope.row.auditStatus || scope.row.auditStatus == '3') ? '#C0C4CC' : '#00A108', padding: '0 0.07rem'}" @click="passApply(scope.row)" type="text" size="small">通过</el-button>
								<el-button :disabled="scope.row.auditStatus == '2' || !scope.row.auditStatus || scope.row.auditStatus == '3'" @click="returnApply(scope.row)" :style="{color: (scope.row.auditStatus == '2' || !scope.row.auditStatus || scope.row.auditStatus == '3') ? '#C0C4CC' : '#FF9829', padding: '0 0.07rem'}" type="text" size="small">退回</el-button>
								<el-button :disabled="scope.row.auditStatus == '2' || !scope.row.auditStatus || scope.row.auditStatus == '1'" :style="{color: (scope.row.auditStatus == '2' || !scope.row.auditStatus || scope.row.auditStatus == '1') ? '#C0C4CC' : '#409EFF', padding: '0 0.07rem'}" @click="recoveryApply(scope.row)" type="text" size="small">恢复</el-button>
							</template>
						</el-table-column>
					</el-table>
				</div>
		...
		<popup v-loading="popupLoading" :popupHeight="'2'" v-if="showPopup" @confim="returnData" @cancel="showPopup = false;resizeAddForm()" :show='showPopup' :title="popupTitle">
			<el-form style="margin:0.2rem 0.2rem 0px 0px;" ref="addForm" :rules="rules" :model="addForm" label-width="120px">
				<el-form-item label="退回原因" prop="returnReason">
					<el-input type="textarea" v-model="addForm.returnReason"></el-input>
				</el-form-item>
			</el-form>
		</popup>
	</div>
	...
	export default {
		...
		methods: {
			...
			init() {
				...
				this.loading = true;
				let requestUrl = '/rule/prop/selectAllAudit',
					data = {
						...this.queryForm,
						flag: 2,
						pageIndex: this.page,
			...
			returnData(){
				...
				let data = {
					...
					ruleExpDate: this.handlerRow.ruleExpDate
				};
				this.$http.post('/rule/prop/AuditOperation', data).then(res => {
					if(res.code == 1){
